    ...
    while ((bytes_read = fread(file_data, 1, DATA_SIZE, fp)) > 0)
    {
        ...
        *(short*)(packet_msg+2) = htons(bytes_read);
        ...
        while (1){
            ...
            tv.tv_sec = 2;//2 * (int)PRRT; // initial timeout is 1 seconds
            ...
            if (bytes_rcvd > 0)
            {
                ...
                short rcvID;
                // rcvID = *(rcv_buffer+1);
                rcvID = (short)ntohs(*(short *)(rcv_buffer));
                printf("receive ACK: rcvID=%d\n", (int)rcvID);
                if ((short)rcvID == (short)sentID)
                {
                    //struct timeval ntv;
